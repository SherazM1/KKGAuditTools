...
from display_compliance.models import DisplayBaseline, ProductRegion
from display_compliance import page as display_page
from display_compliance.segmentation import (
    SegmentationDiagnosticResult,
    SegmentationDiagnostics,
)
from display_compliance.storage import LocalBaselineStorage, baseline_from_dict
...
def test_display_compliance_leaves_global_navigation_to_hub(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    fake_st = _FakeStreamlit(session_state={"hub_route": "display_compliance"})
    monkeypatch.setattr(display_page, "st", fake_st)
    monkeypatch.setattr(display_page, "LocalBaselineStorage", lambda: _FakeDisplayComplianceStorage([]))

    display_page.render()

    assert fake_st.session_state["hub_route"] == "display_compliance"
    assert "page" not in fake_st.session_state
    assert all(
        call["key"] != "display_compliance_back_button"
